    ...
    <div className={`fixed inset-0 bg-[#0a0a0f] text-white flex flex-col items-center justify-center p-4 z-50 overflow-hidden transition-opacity duration-1000 ${isUnlocking ? 'opacity-0' : 'opacity-100'}`}>
      ...
      <div className={`relative z-10 w-full max-w-sm flex flex-col items-center transform transition-all duration-1000 ${mounted ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'}`}>
        ...
        <div className="relative mb-8 group">
          ...
          <div className="relative w-28 h-28 bg-gray-900 border border-gray-700/50 rounded-[2rem] flex items-center justify-center shadow-2xl overflow-hidden">
            <Sparkles className="absolute top-3 right-3 text-yellow-400/50 w-5 h-5 animate-ping" />
            <span className="text-5xl font-extrabold bg-gradient-to-br from-white to-gray-400 bg-clip-text text-transparent transform group-hover:scale-110 transition-transform duration-500 tracking-tighter">
              xAI
            </span>
          </div>
        </div>
        ...
        {/* Title */}
        <h1 className="text-2xl font-bold mb-2 tracking-tight">System Restricted</h1>
        <p className="text-gray-400 text-sm mb-10 text-center max-w-xs leading-relaxed">
          ...
          <div className="transform transition-all duration-300">
            <div className="relative flex items-center">
              ...
              <input 
                ...
                className={`w-full bg-gray-900/60 border ${error ? 'border-red-500/50 focus:border-red-500 shadow-[0_0_15px_rgba(239,68,68,0.2)]' : isUnlocking ? 'border-green-500/50 text-green-400 shadow-[0_0_15px_rgba(34,197,94,0.2)]' : 'border-gray-700 focus:border-blue-500'} rounded-2xl py-4 pl-12 pr-14 text-gray-200 placeholder-gray-500 focus:outline-none focus:ring-1 ${error ? 'focus:ring-red-500' : 'focus:ring-blue-500'} transition-all backdrop-blur-sm`}
                autoFocus
              />
              <button 
                type="submit"
